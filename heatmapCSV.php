...
<html>
  ...
  <body>
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
      ...
      if (($handle = fopen($_FILES["file"]["tmp_name"], "r")) !== FALSE) {
          while (($data = fgetcsv($handle, 10000, ",")) !== FALSE) {
              ...
              while ($cur == 0){
                $key = array_search($_POST["field1"],$data);
                $cur = 3;
              }
              array_push($file, $data[$key]);
          }
          fclose($handle);

          // pre-process of the data array 
    ...
    function initMap() {   
     ...
     for (i in php_var){ 
     var out = php_var[i].slice(1,-1).split(",");
     arr.push(new google.maps.LatLng(out[0],out[1]));
      ...
      function changeGradient() {
        ...
        heatmap.set('ra', heatmap.get('gradient') ? null : gradient);
      }
